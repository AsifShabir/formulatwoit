...
<script type="text/javascript">
    $(document).ready( function(){
        ...
        $(".add-store").click(function() {
            var html = $(".store-row:first").clone();
            html.find("input").val(""); // Clear input values
            $(".store-row:last").after(html);
            if ($(".store-row").length > 1) {
                $(".store-row:last .add-store").removeClass("btn-success add-store").addClass("btn-danger remove-store").html('<i class="fas fa-minus"></i>');
            }
        });

...
namespace App\Http\Controllers\Decathlon;
...
class DecathlonController extends Controller
{
    /**
     * All Utils instance.
     */
     ...
     * Constructor
     *
     * @param  decathlonUtil  $decathlonUtil
     * @return void
     */
    public function __construct(DecathlonUtil $decathlonUtil, ModuleUtil $moduleUtil)
    {
    ...
    public function index()
    {
        try {
            ...
            $woocommerce_api_settings = $this->decathlonUtil->get_api_settings($business_id);
            $alerts = [];
            ...
            $cat_last_sync = $this->decathlonUtil->getLastSync($business_id, 'categories', false);
            ...
            $products_last_synced = $this->decathlonUtil->getLastSync($business_id, 'all_products', false);
        ...
        return view('decathlon.index')->with(compact('tax_rates', 'woocommerce_tax_rates', 'alerts'));
    }
    ...
    public function apiSettings()
    {
        ...
        if (! empty($business->decathlon_api_settings)) {
            $default_settings = json_decode($business->decathlon_api_settings, true);
        ...
        return view('decathlon.api_settings')
    ...
    public function updateSettings(Request $request)
    {
        ...
        try {
            ...
            $business = Business::find($business_id);
            $business->decathlon_api_settings = json_encode($input);
            $business->save();
    ...
    public function syncCategories()
    {
        ...
        $notAllowed = $this->decathlonUtil->notAllowedInDemo();
        if (! empty($notAllowed)) {
            return $notAllowed;
        }
        ...
        try {
            ...
            $this->decathlonUtil->syncCategories($business_id, $user_id);
    ...
    public function syncProducts()
    {
        ...
        try {
            ...
            $limit = 5;
            $all_products = $this->decathlonUtil->syncProducts($business_id, $user_id, $sync_type, $limit, $offset,$nextToken,$markteplaceOffset);
            //dd($all_products);
    ...
    /**
     * Synchronizes pos products Stock with Decathlon stock
     *
     * @return Response
     */
    public function syncStocks()
    {
        ...
        try {
            ...
            $offset = request()->input('offset');
            $limit = 1;
            $all_products = $this->decathlonUtil->syncStocks($business_id, $user_id, $sync_type, $limit, $offset,$nextToken);
            $total_products = count($all_products);
            DB::commit();
            $msg = $total_products > 0 ? __('woocommerce::lang.n_products_synced_successfully', ['count' => $total_products]) : __('woocommerce::lang.synced_successfully');
            $nextToken = request()->session()->get('nextToken');
    ...
    public function syncOrders()
    {
        ...
        try {
            ...
            $this->decathlonUtil->syncOrders($business_id, $user_id);
    ...
    public function getSyncLog()
    {
        ...
        if (request()->ajax()) {
            $last_sync = [
                'categories' => $this->decathlonUtil->getLastSync($business_id, 'categories'),
                'new_products' => $this->decathlonUtil->getLastSync($business_id, 'new_products'),
                'all_products' => $this->decathlonUtil->getLastSync($business_id, 'all_products'),
                'orders' => $this->decathlonUtil->getLastSync($business_id, 'orders'),
    ...
    public function viewSyncLog()
    {
        ...
        if (request()->ajax()) {
            $logs = DecathlonSyncLog::where('decathlon_sync_logs.business_id', $business_id)
                    ->leftjoin('users as U', 'U.id', '=', 'decathlon_sync_logs.created_by')
                    ->select([
                        'decathlon_sync_logs.created_at',
                        'sync_type', 'operation_type',
                        DB::raw("CONCAT(COALESCE(surname, ''), ' ', COALESCE(first_name, ''), ' ', COALESCE(last_name, '')) as full_name"),
                        'decathlon_sync_logs.data',
                        'decathlon_sync_logs.details as log_details',
                        'decathlon_sync_logs.id as DT_RowId',
                    ]);
            $sync_type = [];
            ...
            return Datatables::of($logs)
                ->editColumn('created_at', function ($row) {
                    $created_at = $this->decathlonUtil->format_date($row->created_at, true);
                ...
                ->editColumn('sync_type', function ($row) {
                    $array = [
                        ...
                        'new_decathlon_products' => __('sale.products'),
                        'all_decathlon_products' => __('sale.products'),
        ...
        return view('decathlon.sync_log');
    }
    ...
    public function getLogDetails($id)
    {
        ...
        if (request()->ajax()) {
            $log = DecathlonSyncLog::where('business_id', $business_id)
                                            ->find($id);
            $log_details = json_decode($log->details);

            return view('decathlon.partials.log_details')
    ...
    public function resetCategories()
    {
        ...
        if (request()->ajax()) {
            try {
                ...
                $user_id = request()->session()->get('user.id');
                $this->decathlonUtil->createSyncLog($business_id, $user_id, 'categories', 'reset', null);
    ...
    public function resetProducts()
    {
        ...
        if (request()->ajax()) {
            try {
                ...
                $user_id = request()->session()->get('user.id');
                $this->decathlonUtil->createSyncLog($business_id, $user_id, 'all_products', 'reset', null);

...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        // Trending Products
        $products = $this->productUtil->getTrendingProducts($business_id);
        ...
        $trending_products_chart = new CommonChart;
        $trending_products_chart->labels($labels)
        ...
        // Trending Countries
        $countries = $this->productUtil->getTrendingCountries($business_id,$filters);

        $values = [];
        $labels = [];
        foreach ($countries as $country) {
            $values[] = (float) $country->total_unit_sold;
            $labels[] = $country->country;
        }

        $trending_country_chart = new CommonChart;
        $trending_country_chart->labels($labels)
            ->dataset(__('report.total_unit_sold'), 'column', $values);    
        ...
        return view('home.index', compact('sells_chart_1', 'sells_chart_2', 'widgets', 'all_locations', 'common_settings', 'is_admin','trending_products_chart','trending_country_chart'));
    }

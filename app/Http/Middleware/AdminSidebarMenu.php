    ...
    public function handle($request, Closure $next)
    {
        ...
        Menu::create('admin-sidebar-menu', function ($menu) {
            ...
            if (auth()->user()->can('product.view') || auth()->user()->can('product.create') ||
                ...
                $menu->dropdown(
                    ...
                    function ($sub) {
                        ...
                        $sub->url(
                            action([\App\Http\Controllers\ComponentController::class, 'index']),
                            __('component.components'),
                            ['icon' => 'fa fas fa-gem', 'active' => request()->segment(1) == 'components']
                        );
                    ...
                    function ($sub) use ($enabled_modules, $is_admin, $pos_settings) {
                        ...
                        if ($is_admin || auth()->user()->hasAnyPermission(['sell.view', 'sell.create', 'direct_sell.access', 'direct_sell.view', 'view_own_sell_only', 'view_commission_agent_sell', 'access_shipping', 'access_own_shipping', 'access_commission_agent_shipping'])) {
                            $sub->url(
                                ...
                                ['icon' => 'fa fas fa-list', 'active' => request()->segment(1) == 'sells' && request()->segment(2) == null && request()->input('list') == null]
                            );
                        }
                        if (in_array('add_sale', $enabled_modules) && auth()->user()->can('direct_sell.access')) {
                            $sub->url(
                                action([\App\Http\Controllers\SellController::class, 'create'], ['status' => 'invoice']),
                                __('sale.add_sale'),
                                ['icon' => 'fa fas fa-plus-circle', 'active' => request()->segment(1) == 'sells' && request()->segment(2) == 'create' && request()->get('status') == 'invoice']
                            );
                        }

                        if ($is_admin || auth()->user()->hasAnyPermission(['sell.view', 'sell.create', 'direct_sell.access', 'direct_sell.view', 'view_own_sell_only', 'view_commission_agent_sell', 'access_shipping', 'access_own_shipping', 'access_commission_agent_shipping'])) {
                            $sub->url(
                                action([\App\Http\Controllers\SellController::class, 'index'],['list' => 'invoices'] ),
                                __('List Invoices'),
                                ['icon' => 'fa fas fa-list', 'active' => request()->segment(1) == 'sells' && request()->input('list') == 'invoices']
                            );
                        }
                        ...
                        if (in_array('add_sale', $enabled_modules) && auth()->user()->can('direct_sell.access')) {
                            $sub->url(
                                action([\App\Http\Controllers\SellController::class, 'create'], ['status' => 'delivery_note']),
                                __('Add Delivery Note'),
                                ['icon' => 'fa fas fa-plus-circle', 'active' => request()->segment(1) == 'sells' && request()->segment(2) == 'create' && request()->get('status') == 'delivery_note']
                            );
                        }

                        if ($is_admin || auth()->user()->hasAnyPermission(['sell.view', 'sell.create', 'direct_sell.access', 'direct_sell.view', 'view_own_sell_only', 'view_commission_agent_sell', 'access_shipping', 'access_own_shipping', 'access_commission_agent_shipping'])) {
                            $sub->url(
                                action([\App\Http\Controllers\SellController::class, 'index'],['list' => 'delivery_notes'] ),
                                __('List Delivery Notes'),
                                ['icon' => 'fa fas fa-list', 'active' => request()->segment(1) == 'sells' && request()->input('list') == 'delivery_notes']
                            );
                        }
                    ...
                    function ($sub) use ($enabled_modules, $is_admin) {
                        ...
                        if ((in_array('purchases', $enabled_modules) || in_array('add_sale', $enabled_modules) || in_array('pos_sale', $enabled_modules)) && auth()->user()->can('purchase_n_sell_report.view')) {
                            $sub->url(
                                ...
                                ['icon' => 'fa fas fa-exchange-alt', 'active' => request()->segment(2) == 'sell-report']
                            );
                        }
                        ...
                        if (auth()->user()->can('contacts_report.view')) {
                            $sub->url(
                                ...
                                __('report.contacts'),
                                ['icon' => 'fa fas fa-address-book', 'active' => request()->segment(2) == 'customer-report']
                            );
                            $sub->url(
            ...
            if (auth()->user()->can('business_settings.access') ||
                ...
                $menu->url(
                    action([\App\Http\Controllers\Miravia\MiraviaController::class, 'index']),
                    __('Miravia'),
                    ['icon' => 'fa fa-shopping-bag', 'style' => config('app.env') == 'demo' ? 'background-color: #9E458B !important;' : '', 'active' => request()->segment(1) == 'miravia']
                )->order(89);

                // $menu->url(
                //     action([\App\Http\Controllers\Decathlon\DecathlonController::class, 'index']),
                //     __('woocommerce::lang.decathlon'),
                //     ['icon' => 'fa fa-shopping-bag', 'style' => config('app.env') == 'demo' ? 'background-color: #9E458B !important;' : '', 'active' => request()->segment(1) == 'decathlon']
                // )->order(90);

                $menu->url(
                    action([\App\Http\Controllers\Amazon\AmazonController::class, 'index']),
                    __('woocommerce::lang.amazon'),
                    ['icon' => 'fa fa-shopping-bag', 'style' => config('app.env') == 'demo' ? 'background-color: #9E458B !important;' : '', 'active' => request()->segment(1) == 'amazon']
                )->order(90);

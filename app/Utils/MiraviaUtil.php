...
class MiraviaUtil extends Util
{
    ...
    public function syncOrders($business_id, $user_id)
    {
        ...
        foreach ($orders as $key=>$order) {
            ...
            // if($order['order_number'] == '3047265891549230'){
            //     $created_at = $order['created_at'];
            //     echo $this->convertDateTimeToSpanishTimezone($created_at);
            //     dd($order);
            // }
            ...
            $sell = $miravia_sells->filter(function ($item) use ($order) {
                return $item->delivery_note_number == $order['order_number'];
            })->first();

            $order_number = $order['order_number'];

            $sell_status = 'final';//$this->checkStatus($order['statuses']);
    ...
    public function getMiraviaOrders($business_id)
    {
        ...
        $statuses = ['ready_to_ship', 'delivered','pending'];
        $orders = [];
        $yesterdayDate = Carbon::today()->subDays(2)->format("Y-m-d");
        //$yesterdayDate = "2025-01-09";
        foreach ($statuses as $status) {
            // Prepare the request parameters for fetching orders with the specific status
            $request = new IopRequest('/orders/get', 'GET');
            $request->addApiParam('created_after', $yesterdayDate.'T00:00:00.000Z');
            $request->addApiParam('created_before', $yesterdayDate.'T23:59:59.000Z');
    ...
    public function formatOrderToSale($business_id, $user_id, $order, $sell = null)
    {
        ...
        foreach ($order_items as $product_line) {
            ...
            $product = Product::where('business_id', $business_id)
                ...
                ->with(['variations'])
                ->first();
            //dd($business_id,$product,$product_line);
        ...
        } elseif (!empty($customer_details['name'])) {
            $customer = Contact::where('business_id', $business_id)
        ...
        $new_sell_data = [
            ...
            'transaction_date' => $this->convertDateTimeToSpanishTimezone($order['created_at']),
            'customer_group_id' => $customer->customer_group_id,
            'tax_rate_id' => null,
            'sale_note' => null,
            'commission_agent' => null,
            'invoice_no' => $order['order_number'],
            'delivery_note_number' => $order['order_number'],
        ...
        $payment = [
            ...
            'paid_on' => $this->convertDateTimeToSpanishTimezone($order['created_at']),
        ];

...
class ProductUtil extends Util
{
            ...
            if (! empty($value['variations'])) {
                ...
                foreach ($value['variations'] as $k => $v) {
                    ...
                    $variation_data[] = [
                        ...
                        'product_id' => $product->id,
                        'sub_sku' => $sub_sku,
                        'miravia_variation_id' => $v['miravia_variation_id'],
                        'woocommerce_variation_id' => $v['woocommerce_variation_id'],
                        // 'amazon_variation_id' => $v['amazon_variation_id'],
    ...
    /**
     * Gives list of trending countries
     *
     * @param  int  $business_id
     * @param  array  $filters
     * @return Obj
     */
    public function getTrendingCountries($business_id, $filters = [])
    {
        $query = Transaction::join(
            'transaction_sell_lines as tsl',
            'transactions.id',
    ...
    public function getAmazonVariations($product_id = null)
    {
        // Initialize the raw variations array
        $amazon_variations_raw = [];
    
        if ($product_id != null) {
            $product = Product::find($product_id);
            if ($product) {
                $amazon_product = AmazonProduct::where('sellerSku', $product->amazon_product_id)->first();
                if ($amazon_product) {
                    $amazon_variations_raw = AmazonProduct::where('asin', $amazon_product->asin)
                        ->get(['sellerSku', 'productName', 'condition'])
                        ->toArray();
                }
            }
        } else {
            $amazon_variations_raw = AmazonProduct::get(['sellerSku', 'productName', 'condition'])->toArray();
        }
        
        $amazon_variations = [];
        foreach ($amazon_variations_raw as $ap) {
            $amazon_variations[$ap['sellerSku']] = $ap['productName'] . ' - ' . $ap['condition'] . '_' . $ap['sellerSku'];
        }
        
        return $amazon_variations;
    }

    public function getMiraviaProductVariations($product_id = null)
    {
        $mira_variations = [];
        ...
        if ($product_id !== null) {
            ...
            if ($product) {
                $miravia_raw = MiraviaProduct::where('miravia_id', $product->miravia_product_id)
                    ->get(['sku_id', 'name'])
                    ->toArray();

                foreach ($miravia_raw as $mp) {
                    $mira_variations[$mp['sku_id']] = $mp['name'];
                }
            }
        }

        return $mira_variations;
    }    

    public function getMiraviaVariations($miravia_id = null)
    {
        $miravia_variations = [];
        if ($miravia_id != null) {
            $miravia_variations_raw = MiraviaProduct::where('miravia_id', $miravia_id)->get(['sku_id', 'name'])->toArray();
        
            
            foreach ($miravia_variations_raw as $mp) {
                $miravia_variations[$mp['sku_id']] = $mp['name'];
            }
        }
    
        return $miravia_variations;
    }

    public function getWooVariations($woocommerce_id)
    {
        $woo_variations = [];

        if($woocommerce_id != null){
            $woo_variations_data = WoocommerceProduct::where('woo_id', $woocommerce_id)->pluck('variations')->first();
            $woo_product = WoocommerceProduct::where('woo_id', $woocommerce_id)->first();
            $woo_variations_array = $woo_variations_data ? json_decode($woo_variations_data, true) : [];
            //dd($woo_variations_data,$woo_variations_array);
            if($woo_variations_array){
                foreach ($woo_variations_array as $variation) {
                    $name = $woo_product->name.' ';
                    foreach($variation['attributes'] as $attr){
                        $name .= $attr['option'];
                    }
                    //$woo_variations[$variation['id']] = $variation['name'];
                    $woo_variations[$variation['id']] = $name;
                }
            }
        }

...
class TransactionUtil extends Util
{
    ...
    public function createSellTransaction($business_id, $input, $invoice_total, $user_id, $uf_data = true)
    {
        ...
        $transaction = Transaction::create([
            ...
            'type' => $sale_type,
            'status' => $input['status'],
            'woocommerce_order_id' => $input['woocommerce_order_id'] ?? null,
            'miravia_order_id' => $input['miravia_order_id'] ?? null,
    ...
    public function getReceiptDetails($transaction_id, $location_id, $invoice_layout, $business_details, $location_details, $receipt_printer_type)
    {
        ...
        $output['delivery_note_number'] = $transaction->delivery_note_number;
        ...
        //Order Tax
        $tax = $transaction->tax;
        $output['tax_label'] = "TAX Included";//$invoice_layout->tax_label;
        $output['line_tax_label'] = "TAX Included";//$invoice_layout->tax_label;
        if (! empty($tax) && ! empty($tax->name)) {
            $output['tax_label'] .= ' ('.$tax->name.')';
        }
        $output['tax_label'] .= ':';
        $output['tax'] = ($transaction->tax_amount != 0) ? $this->num_f($transaction->tax_amount, $show_currency, $business_details) : '21%';
    ...
    public function getListSells($business_id, $sale_type = 'sell')
    {
        $sells = Transaction::leftJoin('contacts', 'transactions.contact_id', '=', 'contacts.id')
                ...
                ->select(
                    ...
                    'transactions.woocommerce_store_name',
    ...
    public function getSources($business_id)
    {
        $unique_sources = Transaction::where('business_id', $business_id)
                                    ...
                                    ->groupBy('source')
                                    ->get();
        $sources['woocommerce'] = 'All Woocommerce';

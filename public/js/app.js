$(document).ready(function() {
    ...
    //Start: CRUD for Components
    $(document).on('submit', 'form#component_add_form', function (e) {
        e.preventDefault();
        var form = $(this);
        var data = form.serialize();
        ...
        $.ajax({
            ...
            success: function (result) {
                if (result.success == true) {
                    $('div.components_modal').modal('hide');
                    toastr.success(result.msg);
                    if (typeof components_table !== 'undefined') {
                        components_table.ajax.reload();
                    }
                    var evt = new CustomEvent('componentAdded', { detail: result.data });
                    window.dispatchEvent(evt);
                    //event can be listened as
                    //window.addEventListener("componentAdded", function(evt) {}
                } else {
                    toastr.error(result.msg);
                }
            },
    ...
    $(document).on('click', 'button.edit_component_button', function () {
        $('div.components_modal').load($(this).data('href'), function () {
            $(this).modal('show');

            $('form#component_edit_form').submit(function (e) {
                e.preventDefault();
                var form = $(this);
                var data = form.serialize();
    ...
    $(document).on('click', 'button.delete_component_button', function () {
        swal({
            title: LANG.sure,
            text: LANG.confirm_delete_component,
            icon: 'warning',
            buttons: true,
            dangerMode: true,
        }).then((willDelete) => {
    ...
    $(document).on('click', '#add_variation', function() {
        ...
        $.ajax({
            ...
            data: { row_index: row_index, action: action, product_id:product_id },
            dataType: 'html',
            success: function(result) {
                if (result) {

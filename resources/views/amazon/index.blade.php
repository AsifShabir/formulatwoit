@extends('layouts.app')
@section('title', 'Amazone')

@section('content')
@include('amazon.layouts.nav')
                            ...
                            <table class="table">
                                <thead>
                                    <tr>
                                        ...
                                        <th>Equivalent Amazon Tax Rate</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    @if(!empty($tax_rates))
...
<script type="text/javascript">
    ...
    function sync_products(btn, btn_html, offset = 0, nextToken = '', markteplaceOffset = 0) {
        ...
        $.ajax({
            url: "{{action([\App\Http\Controllers\Amazon\AmazonController::class, 'syncProducts'])}}?type=" + type + "&offset=" + offset+"&nextToken="+nextToken+"&markteplaceOffset="+markteplaceOffset,
            dataType: "json",
            timeout: 0,
            success: function(result){
                if(result.success){
    ...
    function sync_stocks(btn, btn_html, offset = 0, nextToken = '') {
        var type = 'new';
        $.ajax({
            url: "{{action([\App\Http\Controllers\Amazon\AmazonController::class, 'syncStocks'])}}?type=" + type + "&offset=" + offset+"&nextToken="+nextToken,
            dataType: "json",
            timeout: 0,
            success: function(result){
                if(result.success){

...
<script type="text/javascript">
    $(document).ready( function () {
        var sync_log_table =  $('#sync_log_table').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{action([\App\Http\Controllers\Amazon\AmazonController::class, 'viewSyncLog'])}}",
            "order": [[ 1, "desc" ]],
            columnDefs: [ {
                "targets": 5,
                "orderable": false
            ...
            columns: [
                ...
                {data: 'created_at', name: 'amazon_sync_logs.created_at'},
                {data: 'sync_type', name: 'sync_type'},
                {data: 'operation_type', name: 'operation_type'},
                {data: 'full_name', name: 'full_name'},
                {data: 'data', name: 'amazon_sync_logs.data'},
            ],
    ...
    function get_log_details ( rowData ) {
        var div = $('<div/>')
            ...
            .text( 'Loading...' );
        $.ajax( {
            url: '/amazon/get-log-details/' + rowData.DT_RowId,
            dataType: 'html',

...
<section class="content">
  ...
  <div class="row">
    ...
    <div class="col-sm-4">
      <div class="form-group">
        {!! Form::label('woocommerce_product_id', __('product.woocommerce_products') . ':') !!}
        {!! Form::select('woocommerce_product_id', $woo_products, $product->woocommerce_product_id, ['placeholder' => __('messages.please_select'), 'class' => 'form-control select2', 'id' => 'woocommerce_product_id']) !!}
      </div>
    </div>

    <div class="col-sm-4">
      <div class="form-group">
        {!! Form::label('miravia_product_id', __('product.miravia_products') . ':') !!}
        {!! Form::select('miravia_product_id', $miravas, $product->miravia_product_id, ['placeholder' => __('messages.please_select'), 'class' => 'form-control select2', 'id' => 'miravia_product_id']) !!}
      </div>
    </div>

    <div class="col-sm-4">
      <div class="form-group">
        {!! Form::label('amazon_product_id', __('product.amazon_products') . ':*') !!}
        {!! Form::select('amazon_product_id', $amazon_products, $product->amazon_product_id, ['placeholder' => __('messages.please_select'), 'class' => 'form-control select2']); !!}
      </div>
    </div>

    <div class="col-sm-4">
      <div class="form-group">
        {!! Form::label('decathlon_product_id', __('Decathlon Products') . ':') !!}
        {!! Form::select('decathlon_product_id', $decathlon_products, $product->decathlon_product_id, ['placeholder' => __('messages.please_select'), 'class' => 'form-control select2', 'id' => 'decathlon_product_id']) !!}
      </div>
    </div>
    ...
    <div class="col-sm-4" id="alert_quantity_div" @if(!$product->enable_stock) style="display:none" @endif>
      <div class="form-group">
        {!! Form::label('alert_quantity', __('product.alert_quantity') . ' (TORRERO):') !!} @show_tooltip(__('tooltip.alert_quantity'))
    ...
    <div class="col-sm-4" id="alert_quantity_div" @if(!$product->enable_stock) style="display:none" @endif>
      <div class="form-group">
        {!! Form::label('alert_quantity', __('product.alert_quantity') . ' (MYPA):') !!} @show_tooltip(__('tooltip.alert_quantity'))
        {!! Form::text('alert_quantity_mypa', $alert_quantity_mypa, ['class' => 'form-control input_number',
        'placeholder' => __('product.alert_quantity') , 'min' => '0']); !!}
      </div>
    </div>
    <div class="col-sm-4" id="alert_quantity_amazon_div" @if(!$product->enable_stock) style="display:none" @endif>
      <div class="form-group">
        {!! Form::label('alert_quantity_amazon', __('product.alert_quantity_amazon') . ':') !!} @show_tooltip(__('tooltip.alert_quantity'))
        {!! Form::text('alert_quantity_amazon', $alert_quantity_amazon, ['class' => 'form-control input_number',
        'placeholder' => __('product.alert_quantity_amazon') , 'min' => '0']); !!}
      </div>
    </div>

    <div class="col-sm-4">
      <div class="form-group">
        {!! Form::label('product_components', __('component.components') . ':') !!} @show_tooltip(__('component.component_help'))
        {!! Form::select('product_components[]', $product_components, $product->components->pluck('id'), ['class' => 'form-control select2', 'multiple', 'id' => 'product_components']); !!}
      </div>
    </div>
...
<script type="text/javascript">
  $(document).ready(function() {
    ...
    setTimeout(() => {
      $("#add_variation").data('product_id', product_id);
    }, 1000);

    $("#amazon_product_id").on('change', function() {
      $("#add_variation").data('product_id', $(this).val());
    })

    // miravia product type
    $('#miravia_product_id').change(function() {
      var miraviaProductId = $(this).val();

      if (miraviaProductId) {
        $.ajax({
          url: "{{ route('miravia.getProductType') }}",
          method: 'GET',
          data: {
            miravia_product_id: miraviaProductId
          },
          success: function(result) {
            var type = result.product_type;

            // Set the type select input value
    ...
    $('#woocommerce_product_id').change(function() {
      var wooProductId = $(this).val();

      if (wooProductId) {
        $.ajax({
          url: "{{ route('woocommerce.getProductType') }}",
          method: 'GET',
          data: {
            woocommerce_product_id: wooProductId
          },
          success: function(result) {
            console.log('getProductType', result);
            var type = result.product_type;
